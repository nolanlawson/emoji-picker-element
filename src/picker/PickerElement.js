...
export default class PickerElement extends HTMLElement {
  ...
  _set (prop, newValue) {
    ...
    if (['locale', 'dataSource'].includes(prop)) {
      // Wait a microtask in case both of them change. We don't want to create two separate
      // Databases when both props/attrs change in the same event loop tick.
      queueMicrotask(() => this._dbCreate())
    }
  }
  ...
  _dbCreate () {
    ...
    if (!database || database.locale !== locale || database.dataSource !== dataSource) {
      ...
      console.info('new db', locale, dataSource)
