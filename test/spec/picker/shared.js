...
export const openSkintoneListbox = async (container, skipActiveElementCheck) => {
  await waitFor(() => expect(getByRole(container, 'button', { name: 'Choose a skin tone (currently Default)' }))
    .toBeVisible())
  expect(queryAllByRole(container, 'listbox', { name: 'Skin tones' })).toHaveLength(0)
  await fireEvent.click(getByRole(container, 'button', { name: 'Choose a skin tone (currently Default)' }))
